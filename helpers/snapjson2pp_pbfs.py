...
def convert_json_to_adjlist(input_filename, output_filename):
    """
    ...
    if not isinstance(graph_data, dict):
        raise TypeError(
            f"Unsupported JSON structure: Top level must be a Dictionary, but found {type(graph_data)}."
        )
    ...
    for source_node, targets in graph_data.items():
        ...
        if not isinstance(targets, list):
            print(
                f"WARNING: Value for source node '{source_node}' is not a List. Ignoring. Value={targets}",
                file=sys.stderr,
            )
            continue
    ...
    try:
        ...
        print(
            "INFO: Sorting nodes alphabetically (some IDs may not be integers).",
            file=sys.stderr,
        )
    ...
    with open(output_filename, "w") as fout:
        ...
        for node_id in sorted_nodes:
            ...
            neighbours_str = " ".join([f"{v}" for v in neighbours_list])
