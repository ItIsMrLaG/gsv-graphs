...
public class CSVIntEdgeMetaInputFormat extends TextEdgeInputFormat<IntWritable, EdgeMeta> {
    ...
    @Override
    public EdgeReader<IntWritable, EdgeMeta> createEdgeReader(InputSplit split, TaskAttemptContext context) throws IOException {
...
//      OTHER META UNSUPPORTED ('a', 'p')
            ...
            IntWritable from = new IntWritable(Integer.parseInt(tokens[1]));
            IntWritable to = new IntWritable(Integer.parseInt(tokens[2]));
            IntWritable label = new IntWritable(Integer.parseInt(tokens[3]));
