...
public class Boruvka extends BasicComputation<
        IntWritable, BoruvkaVertexValue, IntWritable, BoruvkaMsg> {

    private static final Logger LOG =
            Logger.getLogger(Boruvka.class);

    @Override
    public void compute(
            Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
            Iterable<BoruvkaMsg> messages
    ) throws IOException {
    ...
    void phase1_choseMinEdgeAndSendId(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                      Iterable<BoruvkaMsg> messages) {
        ...
        for (Edge<IntWritable, IntWritable> edge : vertex.getEdges()) {
            ...
            if (minEdge == null || label < minEdge.getValue().get())
                minEdge = edge;

            if (edge.getValue().get() < label)
                minEdge = edge;
        ...
        myValue.minEdgeTriple = new EdgeTriple(vertex.getId(), minEdge.getValue(), minEdge.getTargetVertexId());
    ...
    void phase2_getResponseFromChosenId(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                        Iterable<BoruvkaMsg> messages) {
        BoruvkaVertexValue myValue = vertex.getValue();
    ...
    void phase3_findingSuperVertex(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                   Iterable<BoruvkaMsg> messages) {
        BoruvkaVertexValue myValue = vertex.getValue();
        ...
        switch (myValue.type) {
            case UNKNOWN:
                ...
                for (BoruvkaMsg msg : messages) {
                    ...
                    if (mbSuperVertexId == PHASE3_REQUEST_SUPER_V_ID.code)
                        continue;
    ...
    void phase4_requestUpdateGraphEdges(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                        Iterable<BoruvkaMsg> messages) {
    ...
    void phase5_responseUpdateGraphEdges(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                         Iterable<BoruvkaMsg> messages) {
        for (BoruvkaMsg msg : messages) {
    ...
    void phase6_updateGraphEdges(Vertex<IntWritable, BoruvkaVertexValue, IntWritable> vertex,
                                 Iterable<BoruvkaMsg> messages) {
...
//
//        for (BoruvkaMsg msg : messages) {
//            LOG.assertLog(msg.superVertexResponse.get() == PHASE4_DEFAULT.code, "Strange message2");
//            LOG.assertLog(msg.superVertexResponse.get() == PHASE45_REQUEST_SUPER_V_ID.code, "Strange message3");
//
